...
# delete old entries
file_list = [os.path.join('temp-logs', f) for f in os.listdir("temp-logs") if f.endswith(".txt")]
for f in file_list:
    os.remove(f)
...
file_list = [os.path.join('temp-logs', f) for f in sorted(os.listdir("temp-logs"), key=natural_keys) if f.endswith(".txt")]
